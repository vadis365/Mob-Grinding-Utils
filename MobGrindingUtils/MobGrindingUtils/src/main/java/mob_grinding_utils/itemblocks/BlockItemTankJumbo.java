...
public class BlockItemTankJumbo extends BlockItem {
	...
	public BlockItemTankJumbo(Block blockIn, Properties builder) {
	...
	public void appendHoverText(ItemStack stack, @Nullable Level world, @Nonnull List<Component> list, @Nonnull TooltipFlag flag) {
		if(stack.hasTag() && !stack.getTag().contains("Empty")) {
			...
			if(fluid !=null) {
				...
				list.add(Component.literal(""+ fluid.getAmount() +"Mb/1024000Mb").withStyle(ChatFormatting.BLUE));
			}
		}
		else
			list.add(Component.literal("Holds 1024000Mb (1024 Buckets)").withStyle(ChatFormatting.BLUE));
	}
