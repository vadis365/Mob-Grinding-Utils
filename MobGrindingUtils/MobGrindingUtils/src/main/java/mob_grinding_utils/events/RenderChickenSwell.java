		...
		if (event.getEntity() instanceof Chicken) {
			...
			if (event.getEntity().getPersistentData().contains("shouldExplode")) {
				...
				if (event.getRenderer().getModel() instanceof ChickenModel) {
					...
					model.renderToBuffer(event.getPoseStack(), event.getMultiBufferSource().getBuffer(RenderType.entitySolid(RL.rl("minecraft","textures/entity/chicken.png"))), event.getPackedLight(), OverlayTexture.NO_OVERLAY, 1F, 1F, 1F, 1F);
